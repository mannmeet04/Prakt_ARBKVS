...
void sevenSeg_displayFloat(float value)
{
    ...
    // 12.7 -> Soll "13" anzeigen (ohne Punkt)
    if (value >= 10.0f) {
        // 12.7 + 0.5 = 13.2 -> (int)13
        uint8_t temp = (uint8_t)(value + 0.5f);
        
        // Sicherheits-Check: Display kann max "99" anzeigen
        if (temp > 99) temp = 99; 

        sevenSegValue = temp; 
        sevenSegDot = false; // WICHTIG: Punkt aus!
    } else {

    // value mal 10, da sonst geht die nach kommastelle verloren
    // bei 24,6 -> 25,1 = 25
    uint8_t temp = (uint8_t)((value * 10.0f) + 0.5f);
    //speicherung von wert
    sevenSegValue = temp; 
    sevenSegDot=true; 
}
}